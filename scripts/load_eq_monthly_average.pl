    ...
    my $end_month  = Utils::get_curr_month();
    my $base_query = "SELECT avg(close) FROM nse.equities_eod_data where TIMESTAMP between ";
    my $count = 0;
   # print "Start year = $start_year and End Year = $end_year\n";
   # print Data::Dumper::Dumper $arry_ref;
    foreach my $symbol (@equities) {
        ...
        for(my $yr = $start_year; $yr <= $end_year; $yr++){
            for(my $mon=1; $mon<=12; $mon++){
                # print "Count = $previous_avg_calc_count   ---  PREV = $previous_avg_increase_percent\n";
                my $month = $mon; # this is required as $mon value changes
                last if (($yr == $end_year) && ($mon > $end_month));
                my $from_date = Utils::get_DB_date($yr,$month);
                my $to_date   = Utils::get_DB_date($yr,++$month);
                ...
                #
                $sth = $dbh->prepare("INSERT INTO $table_name (SYMBOL, YEAR, MONTH, AVERAGE, INCREASE_PERCENT, 
                               ...
                               VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
                # print "SYMBOL = $symbol->[0], YEAR = $yr, MONTH = $mon, DATA = $data, INCREASE = $inc_percent\n";
                $sth->execute($symbol->[0], $yr, $mon, $data, $inc_percent, 
                              $inc_percent > 10  ? 1 : 0,
                              $inc_percent > 30  ? 1 : 0,
                              $inc_percent > 50  ? 1 : 0,
                              $inc_percent > 70  ? 1 : 0,
